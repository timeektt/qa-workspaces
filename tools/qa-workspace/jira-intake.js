...
(function () {
  ...
  function renderThumbs() {
    $('jin-thumbs').innerHTML = images.map((im, i) => {
      if (isImg(im.name)) {
        return `<div class="jin-thumb"><img src="${im.dataUri}" alt="${esc(im.name)}" title="คลิกเพื่อดูภาพเต็ม" data-i="${i}"><button class="jin-rm" data-i="${i}" title="ลบไฟล์">✕</button></div>`;
      }
  ...
  async function addFiles(fileList) {
    ...
    renderThumbs();
    if (rejected.length && $('jin-note')) $('jin-note').textContent = '✗ ข้ามไฟล์: ' + rejected.join(', ');
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    const fileInput = $('jin-file-input');
